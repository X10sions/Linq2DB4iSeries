	...
	public partial class DB2iSeriesSqlBuilder : BasicSqlBuilder {
		...
		public DB2iSeriesSqlProviderFlags DB2iSeriesSqlProviderFlags {
			get {
				if(db2iSeriesSqlProviderFlags is null)
					db2iSeriesSqlProviderFlags = new DB2iSeriesSqlProviderFlags(Provider.ProviderOptions.DB2iSeriesVersion, SqlProviderFlags);
		...
		protected override void BuildColumnExpression(SelectQuery selectQuery, ISqlExpression expr, string alias, ref bool addAlias) {
			...
			if(expr is SqlValue value && value.Value == null) {
				var colType = MappingSchema.GetDbTypeForCast(new SqlDataType(value.ValueType), Provider.ProviderOptions.DB2iSeriesVersion).ToSqlString();
		...
		protected override string Comma => Provider.ProviderOptions.ProviderType.IsOleDb() ? ", " : base.Comma;

		//OleDb provider needs spaces in specific places
		protected override string InlineComma => Provider.ProviderOptions.ProviderType.IsOleDb() ? ", " : base.InlineComma;

		//OleDb provider needs spaces in specific places
		protected override string OpenParens => Provider.ProviderOptions.ProviderType.IsOleDb() ? "( " : base.OpenParens;
		...
		protected override void BuildDataTypeFromDataType(SqlDataType type, bool forCreateTable) {
			var dbType = MappingSchema.GetDbDataType(type.SystemType, type.Type.DataType, type.Type.Length, type.Type.Precision, type.Type.Scale, forCreateTable, Provider.ProviderOptions.DB2iSeriesVersion);
		...
		protected override StringBuilder BuildExpression(ISqlExpression expr, bool buildTableName, bool checkParentheses, string alias, ref bool addAlias, bool throwExceptionIfTableNotFound = true) {
			...
			if(expr is SqlParameter parameter && parameter.Name != null) {
				var typeToCast = MappingSchema.GetDbTypeForCast(new SqlDataType(parameter.Type), Provider.ProviderOptions.DB2iSeriesVersion);
		...
		protected override void BuildTypedExpression(SqlDataType dataType, ISqlExpression value) {
			...
			var typeToCast = value switch {
				SqlParameter sqlParameter when sqlParameter.Name != null => MappingSchema.GetDbTypeForCast(dataType, Provider.ProviderOptions.DB2iSeriesVersion),
				SqlValue _ => MappingSchema.GetDbTypeForCast(dataType, Provider.ProviderOptions.DB2iSeriesVersion),
